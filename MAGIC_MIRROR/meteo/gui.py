...
class LiveWeather:
    ...
    def update_weather(self):
        ...
        self.we_label_temp.after(1000, self.update_weather)

        self.we_label_wind.config(text=f"Windspeed: {data[1]} m/s")
        self.we_label_wind.after(1000, self.update_weather)
        
        self.we_label_code.config(text=f"Weather Code: {data[2]}")
        self.we_label_code.after(1000, self.update_weather)

        self.we_label_dir.config(text=f"Wind direction: {data[3]}°")
        self.we_label_dir.after(1000, self.update_weather)
